...
import { verificarTokenFatura } from "@/lib/fatura-token";
...
export default async function FaturaPublicaPage({ params }: Props) {
  const { token } = await params;

  const ids = verificarTokenFatura(token);
  if (!ids) notFound();

  const { recebivelId, empresaId } = ids;

  const [recebivel, empresa] = await Promise.all([
    prisma.recebivel.findFirst({
      where: { id: recebivelId, empresa_id: empresaId },
      include: { cliente: true, contrato: true, conta_bancaria: true },
    }),
        ...
        <div className="p-10 print:p-8 space-y-8">
          ...
          <div className="flex items-start justify-between gap-6 pb-6 border-b-2 border-[#1B4F72]">
            <div className="flex-1">
              ...
              {nomeFantasia && <p className="text-sm text-slate-600">{nomeFantasia}</p>}
              {empresa.cnpj && <p className="text-sm text-slate-500">CNPJ: {empresa.cnpj}</p>}
              {enderecoEmpresa && <p className="text-xs text-slate-500 mt-1">{enderecoEmpresa}</p>}
          ...
          {cliente && (
            <div className="grid grid-cols-2 gap-6">
              <div className="bg-slate-50 rounded-lg p-4">
                ...
                {cliente.nome_fantasia && <p className="text-sm text-slate-600">{cliente.nome_fantasia}</p>}
                {cliente.cpf_cnpj && <p className="text-sm text-slate-500 mt-1">CNPJ/CPF: {cliente.cpf_cnpj}</p>}
              ...
              {contrato && (
                <div className="bg-slate-50 rounded-lg p-4">
                  ...
                  {contrato.numero_contrato && <p className="text-sm text-slate-500">Nº {contrato.numero_contrato}</p>}
                  {contrato.objeto && <p className="text-xs text-slate-500 mt-1 line-clamp-3">{contrato.objeto}</p>}
                  <div className="mt-2 text-xs text-slate-500 space-y-0.5">
                    {contrato.data_inicio && <p>Início: {fmtDate(contrato.data_inicio)}</p>}
                    {contrato.data_fim && <p>Término: {fmtDate(contrato.data_fim)}</p>}
                    {contrato.forma_pagamento && <p>Pagamento: {contrato.forma_pagamento}</p>}
                    {contrato.periodicidade && <p>Periodicidade: {contrato.periodicidade.toLowerCase()}</p>}
                    {contrato.valor_total && Number(contrato.valor_total) > 0 && (
                      <p>Valor total: <strong>{fmtBRL(Number(contrato.valor_total))}</strong></p>
                    )}
                  </div>
                </div>
              )}
            ...
            <table className="w-full border border-slate-200 rounded-lg overflow-hidden text-sm">
              ...
              <tbody>
                <tr className="border-t border-slate-100">
                  ...
                  <td className="px-4 py-3 text-center text-slate-600">{fmtDate(recebivel.data_vencimento)}</td>
                  <td className="px-4 py-3 text-right font-semibold text-slate-800">{fmtBRL(valor)}</td>
                </tr>
              </tbody>
            </table>
          </div>
                  ...
                  <div className="text-sm text-slate-700 space-y-0.5">
                    ...
                    {contaRecebimento.conta && (
                      <p>
                        ...
                        {contaRecebimento.digito ? `-${contaRecebimento.digito}` : ""}
